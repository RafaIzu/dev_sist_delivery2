...
from wtforms import StringField, SubmitField, SelectField, FloatField, TextField, IntegerField
...
class AddProduct(FlaskForm):
    ...
    description = TextField("Descrição", validators=[DataRequired()])
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.theme.choices = [(theme.id, theme.name)
                              for theme in Theme.query.order_by(Brand.name).all()]
        self.category.choices = [(category.id, category.name)
                                 for category in Category.query.order_by(Brand.name).all()]
